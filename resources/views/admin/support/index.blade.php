...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex justify-content-between">
            ...
            <div>
                @can('slider-add')
                <a href="{{ route('supports.create') }}" class="btn btn-primary btn-sm"><i class="fa-solid fa-circle-plus"></i> Thêm mới</a>
                @endcan
            </div>
        </div>
            ...
            <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0">
                <thead>
                    <tr>
                        ...
                        <th class="col-sm-3 text-center">Tiêu đề</th>
                        <th class="col-sm-2 text-center">Vị trí</th>
                ...
                <tbody>
                    ...
                    @else
                    @foreach ($supports as $val)
                    <tr>
                        <td>{{ (($supports->currentPage()-1)*config('common.default_page_size')) + $loop->iteration }}</td>
                        <td>{{ $val->name }}</td>
                        <td>{{ $val->title }}</td>
                        <td class="text-center">
                            @if($val->is_role == 1) Chính sách 
                            @else Thông tin khác
                            @endif
                        </td>
                        <td>
                        ...
                        <td class="action">
                            @can('slider-edit')
                            <a href="{{ asset('admin/supports/'.$val->id.'/edit') }}">Chỉnh sửa</a> |
                            @endcan
                            <a href="{{ asset('admin/supports/'.$val->id.'/edit') }}">Xóa cache</a>
                            @can('slider-delete')
                            | <a href="javascript:void(0);" onclick="confirmDelete('{{ $val->id }}')">Xóa</a>
                            <form id="deleteForm-{{ $val->id }}" action="{{ route('supports.destroy', ['id' => $val->id]) }}" method="post" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            @endcan
